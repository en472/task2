...
The files from running the real E.coli data are available in Part2_Data, as well as the files used for attempting to run the simulated data. The real E.coli reads where too large to upload, but are the ones provided in the 'fastq_to_use_at_the_end' folder on CLIMB. 

## Discussion:

It is difficult to determine how differently the two variant callers perform without physically trawling through the data. Direct comparison of the 'quality scores' for each mutation identified are not easily comparable as they have different methods of scoring quality. Generally, both variant callers picked up the same snps, although returned quite different results for INDELS, presumably as these are more complex to identify. The pipeline ran on both the real and simulated samples, and successfully generated results for the Ecoli data, but not the simulated data (I suspect there was a problem with the 'fastq' file formatting?). 
