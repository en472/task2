...
def reads(seq):
    ...
    with open('mutated_sequence_EC.txt', 'r') as file:
    ...
    # empty list for reverse reads
    rev_reads = []

    # generate reverse reads also
    for i in range(0, len(reads)):
        rev_reads.append(reads[i][::-1])
    ...
    with open('reads_PF_1.fq', 'w') as file:
        for line in reads:
            file.write(id + '_' + str(read_number) + '_1' + '\n')
    ...
    # generate new (meaningless) quality scores
    quality_scores = ''.join(random.choices(')*%.!', k = 100))
        
    # re write the file
    with open('reads_PF_2.fq', 'w') as file:
        for line in rev_reads:
            file.write(id + '_' + str(read_number) + '_2' + '\n')
            read_number = read_number + 1
            file.write(line + '\n')
            file.write('+\n')
            file.write(quality_scores + '\n')

reads('mutated_sequence_PF.txt')
